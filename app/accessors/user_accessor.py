...
class UserAccessor(BaseDBAccessor[User]):
    ...
    async def close_active_record(self, user_id: str, session: AsyncSession):
        query = (
            update(User)
    ...
    async def get_active_by_email(self, email: str, session: AsyncSession) -> User | None:
        query = (
            select(User)
            .where(User.user_email == email, User.effective_to.is_(None))
            )
    ...
    async def get_active_by_username(self, username: str, session: AsyncSession) -> User | None:
        query = (
            select(User)
            .where(User.user_name == username, User.effective_to.is_(None))
            )
        result = await session.execute(query)
        return result.scalars().first()
